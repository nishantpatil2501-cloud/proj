...
# Cache the raw connection to keep the app fast
@st.cache_resource
def _get_raw_connection():
    return mysql.connector.connect(
        host="localhost", user="root", password="nishant", database="BankingDB"
    )

# Safely retrieve and wake up the connection if it dropped
def get_db_connection():
    try:
        db = _get_raw_connection()
        # THE MAGIC BULLET: If the connection died, this forces it to reconnect automatically
...
# Enhanced luxury aesthetic palettes
t_colors = {
...
# CSS Injection with Corporate Banking Background
st.markdown(f"""
    <style>
...
# --- 5. ENTERPRISE DASHBOARD ---
    ...
    # Data fallback: Mocks missing columns if your database hasn't been updated yet
    ...
    # --- TAB 2: BANKING SERVICES ---
        ...
        with t1:
            ...
            if st.button("Process Transaction"):
                if mode == "Withdraw" and amt > user_live.get('balance', 0): st.error("Insufficient Funds!")
                elif mode == "Withdraw" and amt > st.session_state.daily_limit: st.error(f"Exceeds Daily Limit of ₹{st.session_state.daily_limit:,.2f}!")
                elif mode == "Withdraw" and not st.session_state.card_active: st.error("Card is secured. Please unlock in settings.")
                else:
                    ...
                    try:
                        cursor.execute("INSERT INTO transactions (account_number, type, amount) VALUES (%s, %s, %s)", (user_live['account_number'], mode, amt))
                    except: pass 
                    db.commit()
        ...
        with t2:
            ...
            if st.button("Initiate NEFT/IMPS"):
                if t_amt > user_live.get('balance', 0): st.error("Insufficient Liquidity.")
                else:
                    cursor.execute("UPDATE customers SET balance = balance - %s WHERE account_number = %s", (t_amt, user_live['account_number']))
                    cursor.execute("UPDATE customers SET balance = balance + %s WHERE account_number = %s", (t_amt, target))
                    try:
                        cursor.execute("INSERT INTO transactions (account_number, type, amount) VALUES (%s, %s, %s)", (user_live['account_number'], "Transfer Out", t_amt))
                    except: pass
                    db.commit()
    ...
    # --- TAB 3: INVESTMENTS ---
        ...
        inv1, inv2 = st.tabs(["📈 Term Deposits", "💼 Equities & Mutual Funds"])
        
        with inv1:
            st.markdown('<div class="yono-card">', unsafe_allow_html=True)
            st.subheader("High-Yield Term Deposit (7.1% p.a.)")
            fd_amt = st.number_input("Principal Amount (₹)", min_value=1000)
            years = st.slider("Term Duration (Years)", 1, 10, 1)
            maturity = fd_amt * ((1 + 0.071)**years)
            st.info(f"Projected Maturity Value: **₹{maturity:,.2f}**")
            if st.button("Lock Capital"):
                if fd_amt > user_live.get('balance', 0): st.error("Insufficient liquidity to fund deposit.")
                else: st.success("Term Deposit Executed. Digital bond dispatched to your encrypted inbox.")
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        with inv2:
            ...
            mf_data = pd.DataFrame({
                "Asset Class": ["Bluechip Equity", "Mid-Cap Growth", "Nifty 50 Index"],
                "Allocated (₹)": [50000, 25000, 100000],
                "Current Valuation (₹)": [58000, 31000, 112000],
                "ROI": ["+16.0%", "+24.0%", "+12.0%"]
            })
    ...
    # --- TAB 4: PREMIUM CARDS ---
        ...
        with col1:
            st.markdown('<div class="yono-card" style="border-left: 4px solid #D4AF37;">', unsafe_allow_html=True)
            st.subheader("The Obsidian Reserve")
            st.write("Hand-milled from titanium. Because feature lists don't sell; exclusive lifestyle experiences do.")
            st.write("• Unlimited Global Airport Lounge Access")
            st.write("• 24/7 Dedicated Lifestyle Concierge")
            st.write("• Zero Forex Markup")
            st.button("Request Upgrade", key="req_obsidian")
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        with col2:
            ...
            st.write("**Status:** Active & Secured")
            st.write("**Reward Points:** 14,250")
            st.button("View Statement")
            st.markdown('</div>', unsafe_allow_html=True)

    # --- TAB 5: MINI STATEMENT ---
        ...
        mock_history = pd.DataFrame({
            ...
            "Description": ["UPI/Zomato", "ATM Withdrawal", "NEFT/Salary", "UPI/Uber", "Spotify Premium"],
            "Type": ["Debit", "Debit", "Credit", "Debit", "Debit"],
            "Amount (₹)": [450.00, 5000.00, 85000.00, 320.00, 119.00]
        })
        
        st.dataframe(mock_history, hide_index=True, use_container_width=True)
        st.download_button("Download Official Statement (PDF)", "Mock PDF Data", file_name="Statement.pdf")
        st.markdown('</div>', unsafe_allow_html=True)

    # --- TAB 6: CONCIERGE SUPPORT ---
        ...
        with c1:
            ...
            st.button("✉️ Email Relationship Manager", use_container_width=True)
        with c2:
            msg = st.text_area("Secure Message to Branch", placeholder="How can we assist you today?")
            if st.button("Send Message"):
                st.success("Message dispatched to Priority Channel.")
        st.markdown('</div>', unsafe_allow_html=True)
